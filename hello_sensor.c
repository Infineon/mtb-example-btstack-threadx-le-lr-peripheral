...
void hello_sensor_interrupt_handler(void *user_data, cyhal_gpio_event_t event );
/*******************************************************************************
...
{
    .callback     = hello_sensor_interrupt_handler,
    .callback_arg = NULL
};
...
static void hello_sensor_fine_timeout(TIMER_PARAM_TYPE count);
                                                ...
                                                wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT:
        ...
        wiced_hal_write_nvram ( HELLO_SENSOR_PAIRED_KEYS_VS_ID, sizeof( wiced_bt_device_link_keys_t ), p_keys ,&wiced_result );
        printf("keys save to NVRAM result: %d \n", wiced_result);
        break;
    ...
    case BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT:
        {
            ...
            printf("keys read from NVRAM req result: %d \n", wiced_result);
    ...
    case BTM_LOCAL_IDENTITY_KEYS_UPDATE_EVT:
        ...
        wiced_hal_write_nvram ( HELLO_SENSOR_LOCAL_KEYS_VS_ID, sizeof( wiced_bt_local_identity_keys_t ), p_keys ,&wiced_result );
        printf("local keys save to NVRAM result: %d \n", wiced_result);
        break;
    ...
    case  BTM_LOCAL_IDENTITY_KEYS_REQUEST_EVT:
        ...
        wiced_hal_read_nvram( HELLO_SENSOR_LOCAL_KEYS_VS_ID, sizeof(wiced_bt_local_identity_keys_t), p_keys, &wiced_result );
        printf("local keys read from NVRAM result: %d \n",  wiced_result);
        break;
...
#define APP_ADV_NAME "Hello"
    ...
    sts = wiced_bt_ble_set_ext_adv_parameters_v2(HELLO_SENSOR_EXT_ADV_HANDLE, &params);
    printf("[%s] wiced_bt_ble_set_ext_adv_parameters_v2 res %d \n", __FUNCTION__, sts);
    ...
    UINT8_TO_STREAM(p_ext_adv_data, strlen(APP_ADV_NAME));
    UINT8_TO_STREAM(p_ext_adv_data, BTM_BLE_ADVERT_TYPE_NAME_COMPLETE);
    ARRAY_TO_STREAM(p_ext_adv_data, (uint8_t *)APP_ADV_NAME, strlen(APP_ADV_NAME));
    ...
    cyhal_gpio_write(CYBSP_USER_LED1, CYBSP_LED_STATE_ON);
    cyhal_gpio_write(CYBSP_USER_LED2, CYBSP_LED_STATE_OFF);
    sts = wiced_bt_ble_start_ext_adv(1, 1, &duration_cfg);
    printf("[%s] wiced_bt_ble_start_ext_adv res %d \n", __FUNCTION__, sts);
}
...
static void hello_sensor_application_init(void)
{
    ...
    wiced_init_timer(&hello_sensor_second_timer, hello_sensor_timeout, 0, WICED_MILLI_SECONDS_PERIODIC_TIMER);
    wiced_start_timer( &hello_sensor_second_timer, HELLO_SENSOR_APP_TIMEOUT_IN_MS );
    ...
    {
        {
            ...
            /* using single VS_ID to support storing only one paired device */
            /* see README for multiple concurrent pairing support */
            bytes_written = wiced_hal_write_nvram( HELLO_SENSOR_VS_ID, sizeof(hello_sensor_hostinfo), (uint8_t*)&hello_sensor_hostinfo, &wiced_result );
                                                                  ...
                                                                  uint16_t len_req)
{
    ...
    /* Fake battery value read increment */
...
static wiced_bt_gatt_status_t hello_sensor_gatts_connection_up(wiced_bt_gatt_connection_status_t *p_status)
{
    ...
    /* Make sure to update phy*/
    {
        ...
        memcpy((void *)phy_pref.remote_bd_addr, (void *)p_status->bd_addr, BD_ADDR_LEN);
...
{
    printf("[%s] %u", __FUNCTION__, (unsigned int)p_buf);
...
static void *hello_sensor_alloc_buffer(int len)
{
    ...
    printf("[%s] len %d alloc %u", __FUNCTION__, len,(unsigned int)p);
 ...
 *
 *******************************************************************************/
void hello_sensor_fine_timeout(TIMER_PARAM_TYPE count)
{
...
static void hello_sensor_load_keys_for_address_resolution(void)
{
    ...
    printf("hello_sensor_load_keys_for_address_resolution  result:%d \n", result );
}
